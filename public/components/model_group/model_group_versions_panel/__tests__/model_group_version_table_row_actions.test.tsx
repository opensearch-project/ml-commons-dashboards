...
describe('<ModelGroupVersionTableRowActions />', () => {
  it('should render actions icon and "Copy ID" and "Delete" button after clicked', async () => {
    const user = userEvent.setup();
    render(<ModelGroupVersionTableRowActions state={MODEL_STATE.uploading} id="1" />);
  ...
  it('should render "Upload new artifact" button for REGISTER_FAILED state', async () => {
    const user = userEvent.setup();
    render(<ModelGroupVersionTableRowActions state={MODEL_STATE.registerFailed} id="1" />);
    await user.click(screen.getByLabelText('show actions'));
  ...
  it('should render "Deploy" button for REGISTERED and UNDEPLOYED state', async () => {
    ...
    const { rerender } = render(
      <ModelGroupVersionTableRowActions state={MODEL_STATE.uploaded} id="1" />
    );
    await user.click(screen.getByLabelText('show actions'));

    expect(screen.getByText('Deploy')).toBeInTheDocument();

    rerender(<ModelGroupVersionTableRowActions state={MODEL_STATE.unloaded} id="1" />);
    expect(screen.getByText('Deploy')).toBeInTheDocument();
  });
  ...
  it('should render "Undeploy" button for DEPLOYED and PARTIALLY_DEPLOYED state', async () => {
    const user = userEvent.setup();
    const { rerender } = render(
      <ModelGroupVersionTableRowActions state={MODEL_STATE.loaded} id="1" />
    );
    await user.click(screen.getByLabelText('show actions'));

    expect(screen.getByText('Undeploy')).toBeInTheDocument();

    rerender(<ModelGroupVersionTableRowActions state={MODEL_STATE.partiallyLoaded} id="1" />);
    expect(screen.getByText('Undeploy')).toBeInTheDocument();
  });

  it('should call close popover after menuitem click', async () => {
    const user = userEvent.setup();
    render(<ModelGroupVersionTableRowActions state={MODEL_STATE.loaded} id="1" />);
